		...
		<div class="mainChap">
			...
			<br>
			 <script>
				...
				function Compute()
				{
				var num1 = Math.floor(Math.random() * 40)+40;
		  	...
		  	$(document).ready(function()
		  	    {
        			...
        			var answer = parseInt($("#Ans").val());
          			  if (answer === total) 
       				 {
       				 		swal({ title: "CONGRATULATIONS!", text: "You can now proceed to chapter 3", icon: "success", button: "Proceed", }) 
							.then((ConChoice)=> {if (ConChoice) { window.open('./Chapter2_Complete.php','_self')  }});
        			 }
        			 else
        			 {
